...
func (vms *VMService) reconcileHardwareVersion(ctx *virtualMachineContext) error {
	if ctx.VSphereVM.Spec.VMXVersion == "" {
		return nil
	}
	...
	toUpgrade, err := util.LessThan(virtualMachine.Config.Version, ctx.VSphereVM.Spec.VMXVersion)
	if err != nil {
	...
	if toUpgrade {
		ctx.Logger.Info("upgrading hardware version",
			...
			"to", ctx.VSphereVM.Spec.VMXVersion)
		task, err := ctx.Obj.UpgradeVM(ctx, ctx.VSphereVM.Spec.VMXVersion)
		if err != nil {
