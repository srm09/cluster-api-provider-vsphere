...
func (v *VSphereMachineTemplateWebhook) ValidateCreate(_ context.Context, raw runtime.Object) error {
	...
	if spec.VMXVersion != "" {
		...
		if !r.MatchString(spec.VMXVersion) {
			allErrs = append(allErrs, field.Invalid(field.NewPath("spec", "template", "spec", "vmxVersion"), spec.VMXVersion, "should be a valid VM hardware version, example vmx-17"))
		}
	}
