...
func (r ServiceAccountReconciler) syncServiceAccountSecret(ctx *vmwarecontext.GuestClusterContext, pSvcAccount vmwarev1.ProviderServiceAccount) error {
	...
	logger.V(4).Info("Fetching svcAccountTokenSecret for provider service account", "svcAccountTokenSecret", secretName)
	...
	if len(svcAccountTokenSecret.Data) == 0 {
		...
		// when a svcAccountTokenSecret is added to the service account by the token controller.
		logger.Info("Skipping sync secret for provider service account: secret has no data", "secret", secretName)
		return nil
	}

	// Choose the default svcAccountTokenSecret
	/*secretRef := svcAccount.Secrets[0]
	logger.V(4).Info("Fetching svcAccountTokenSecret for provider service account", "svcAccountTokenSecret", secretRef.Name)
	...
	logger.V(4).Info("Creating or updating svcAccountTokenSecret in cluster", "namespace", targetSecret.Namespace, "name", targetSecret.Name)
...
func (r ServiceAccountReconciler) secretToVSphereCluster(o client.Object) []reconcile.Request {
	...
	if ownerRef != nil && ownerRef.Kind == kindProviderServiceAccount {
		...
		svcAccountName, _ := secret.GetAnnotations()[corev1.ServiceAccountNameKey]
		svcAccount := &corev1.ServiceAccount{}
